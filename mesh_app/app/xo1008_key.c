...
{
    static uint8_t key_state_his = GPIO_HIGH;
    uint8_t key_state_cur = honyar_gpio_get_input(XO1008_KEY_LOGIC_PIN);
    ...
    if(key_state_his == key_state_cur) {
        if(GPIO_LOW == key_state_his && cur_time - press_time >= LONG_PRESS_TIME_MIN) {
            // long press;
...
AGAIN:
    os_timer_arm(&g_xo1008_key_timer, TASK_CYCLE_TM_MS, false);
}
